...
CREATE TABLE public.Salons(
    ...
    CONSTRAINT Salons_Categories_FK FOREIGN KEY (id_category) 
        REFERENCES public.Categories(id_category) ON DELETE RESTRICT
)WITHOUT OIDS;
...
CREATE TABLE public.Services(
    ...
    CONSTRAINT Services_Salons_FK FOREIGN KEY (id_salon) 
        REFERENCES public.Salons(id_salon) ON DELETE CASCADE
)WITHOUT OIDS;
...
CREATE TABLE public.Reviews(
    ...
    CONSTRAINT Reviews_Salons_FK FOREIGN KEY (id_salon) 
        REFERENCES public.Salons(id_salon) ON DELETE CASCADE
)WITHOUT OIDS;
...
CREATE TABLE public.Users(
    ...
    role         VARCHAR (50) NOT NULL CHECK (role IN ('Admin', 'Coiffeur', 'Client')),
    ...
    CONSTRAINT Users_Salons_FK FOREIGN KEY (id_salon) 
        REFERENCES public.Salons(id_salon) ON DELETE SET NULL
)WITHOUT OIDS;
...
('Coiffeur', 'Jane', 'Smith', 25, 'jane.smith@example.com', '0987654321', 'Lyon', 'password456', current_date, current_date, 2);
...
CREATE TABLE public.Rendez_vous(
    ...
    CONSTRAINT Rendez_vous_Salons_FK FOREIGN KEY (id_salon) 
        REFERENCES public.Salons(id_salon) ON DELETE CASCADE,
    CONSTRAINT Rendez_vous_Users_FK FOREIGN KEY (id_user) 
        REFERENCES public.Users(id_user) ON DELETE CASCADE,
    CONSTRAINT Rendez_vous_Services_FK FOREIGN KEY (id_service) 
        REFERENCES public.Services(id_service) ON DELETE CASCADE
)WITHOUT OIDS;
...
VALUES
(current_date + interval '1 day', '14:30:00', current_timestamp, current_timestamp, 1, 1, 1),
(current_date + interval '2 days', '16:00:00', current_timestamp, current_timestamp, 2, 2, 2);

-- Création des index pour améliorer les performances
CREATE INDEX idx_services_salon ON public.Services(id_salon);
CREATE INDEX idx_reviews_salon ON public.Reviews(id_salon);
CREATE INDEX idx_users_salon ON public.Users(id_salon);
CREATE INDEX idx_rendez_vous_salon ON public.Rendez_vous(id_salon);
CREATE INDEX idx_rendez_vous_user ON public.Rendez_vous(id_user);
CREATE INDEX idx_rendez_vous_service ON public.Rendez_vous(id_service);